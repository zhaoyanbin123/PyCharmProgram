...
def SendReport_Email():
    ...
    msg['Subject'] = Header(u'app自动化测试结果', 'utf-8').encode()
    #---这是文字部分---
    part = MIMEText("This is test report")
    msg.attach(part)
    #---这是附件部分---
    part = MIMEApplication(open(constants.getPath("email_path"), 'rb').read())
    part.add_header('Content-Disposition', 'attachment', filename="Report1.html")
    msg.attach(part)
